...
s.add(b1Data[0] == 0x2f)
s.add(b1Data[1] == 0x2f)
with open("result2_test.txt", "rb") as f:
    out = f.read(0xb00)
    i = 2
    while (i < len(out)):
        if (out[i] == 0x0d):
...
while (s.check() == z3.sat):
    ...
    for a in l_b1Data:
        outData += chr(a)
    with open("step2\\result_%d.txt" % t, "wb") as f:
